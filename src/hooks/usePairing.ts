...
export const usePairing = () => {
  ...
  const createMealPairing = useCallback(
    async (meal: Meal): Promise<PairingRecommendation | null> => {
      setIsLoading(true);
      setError(null);
  ...
  const createPlaylistPairing = useCallback(
    async (playlist: { id: string; name: string }, genre: string) => {
      setIsLoading(true);
      setError(null);
